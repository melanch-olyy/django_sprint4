...
def create_paginator(posts, request, per_page=10):
...
class ArticleMixin:
    ...
    def get_queryset(self):
        ...
        return count_comments(posts).order_by('-pub_date')
...
class CategoryPostsView(ArticleMixin, ListView):
...
class AddFeedbackView(LoginRequiredMixin, FeedbackMixin, CreateView):
...
class EditFeedbackView(LoginRequiredMixin, ModifyFeedbackMixin, UpdateView):
    pass


class RemoveFeedbackView(LoginRequiredMixin, ModifyFeedbackMixin, DeleteView):
    pass
...
class UserProfileView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        posts = count_comments(posts).order_by('-pub_date')
        context['page_obj'] = create_paginator(posts, self.request)
